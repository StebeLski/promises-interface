...
describe('CustomPromise.any', () => {
  ...
  test(`resolves if one of its inputs resolves`, () => {
    ...
    return expect(CustomPromise.any(mockInput)).resolves.toBe(firstResolvedPromiseValue);
  });

  test(`rejects correctly if all of its inputs reject`, () => {
    const mockInput = [scheduleReject(110, 'reject1'), scheduleReject(120, 'reject2'), scheduleReject(130, 'reject3')];
  ...
  test(`saves values to output in order of their position in the input`, () => {
    const mockInput = [scheduleReject(130, 'reject1'), scheduleReject(120, 'reject2'), scheduleReject(110, 'reject3')];
    const mockOutput = ['reject1', 'reject2', 'reject3'];
    return expect(CustomPromise.any(mockInput)).rejects.toMatchObject(mockOutput);
  });
});
